...
# ==================================
# powershell.exe -ep Bypass -noprofile -command (new-object -ComObject wscript.shell).Popup('Installations-Sequenz von Zusatzpaketen beginnt ... ' + $([char]10) + $([char]13) + $([char]10) + $([char]13) + 'Diese Mitteilung wartet 4 Sekunden ...',4,'Information ...',0x0 + 0x40 + 0x1000) >nul 2>&1
# Example for Busybox shell applet with masking of special chars:
# ===============================================================
# powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Installations-Sequenz von Zusatzpaketen beginnt ... \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 4 Sekunden ...\',4,\'Information ...\',0x0 + 0x40 + 0x1000\) >/dev/null 2>&1
...
if ! test -d "$APPDATA/pandoc/media"; then
  echo "Verzeichnis \"$APPDATA/pandoc/media\" wird angelegt ..."
  mkdir -p "$APPDATA/pandoc/media" >/dev/null 2>&1
  if test -d "$APPDATA/pandoc/media"; then
    echo "Erolgreich: Verzeichnis angelegt !"
    # ################################################################
    ...
    if test ${CHDIRTO} == "false"; then
      echo "Abbruch: Fehler beim Wechsel in das Verzeichnis \"$APPDATA/pandoc\" !"
      powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Abbruch: Fehler beim Wechsel in das Verzeichnis APPDATA/pandoc ! \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 6 Sekunden ...\',6,\'Fehler !\',0x0 + 0x10 + 0x1000\) >/dev/null 2>&1
      # read a
      exit 1
    else
      echo "Wechsel in Verzeichnis..: \"$APPDATA/pandoc\" erfolgreich, fahre fort ..."
    fi
  else 
    echo "Abbruch: Verzeichnis \"$APPDATA/pandoc/media\" wurde NICHT angelegt !"
    powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Abbruch: Das Verzeichnis APPDATA/pandoc/media konnte NICHT angelegt werden ! \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 6 Sekunden ...\',6,\'Fehler !\',0x0 + 0x10 + 0x1000\) >/dev/null 2>&1
    # read a
    exit 1
  fi
else 
    echo "Verzeichnis \"$APPDATA/pandoc/media\" existiert bereits ..."
    # ################################################################
    # WE HAVE TO CHANGE INTO A WRITABLE "curdir working directory"
    # FROM WHICH "pandoc" IS RUN, BECAUSE PANDOC WILL CREATE SOME
    # RANDOM-FILES HERE, DURING PDF-CREATION (i. e.: toP25FA.pdf,
...
# Splash-Message
powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Pandoc Gui by Veit Berwig, ZIT-SH 2025\' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Die Konvertierung von Dokumenten mit Pandoc beginnt ... \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 6 Sekunden ...\',6,\'Information ...\',0x0 + 0x40 + 0x1000\) >/dev/null 2>&1
...
if test ${ZENITY} == "false"; then
  echo "Abbruch: Zenity muss im Suchpfad gefunden werden, damit dieses Skript verwendet werden kann !"
  powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Abbruch: Zenity muss im Suchpfad gefunden werden, damit dieses Skript verwendet werden kann ! \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 6 Sekunden ...\',6,\'Fehler !\',0x0 + 0x10 + 0x1000\) >/dev/null 2>&1
  # read a
  exit 1
fi

if test ${PANDOC} == "false"; then
  echo "Abbruch: Pandoc muss im Suchpfad gefunden werden, damit dieses Skript verwendet werden kann !"
  powershell.exe -ep Bypass -noprofile -command \(new-object -ComObject wscript.shell\).Popup\(\'Abbruch: Pandoc muss im Suchpfad gefunden werden, damit dieses Skript verwendet werden kann ! \' + \$\(\[char\]10\) + \$\(\[char\]13\) + \$\(\[char\]10\) + \$\(\[char\]13\) + \'Diese Mitteilung wartet 6 Sekunden ...\',6,\'Fehler !\',0x0 + 0x10 + 0x1000\) >/dev/null 2>&1
  # read a
  exit 1
fi
...
if [ $1 ] && [ -s "$*" ]; then
  ...
  if ! SOURCEFILE=$(${ZENITYWIN} --file-selection --title="Bitte wählen Sie die Datei, die  mit \"Pandoc\" konvertiert werden soll:");  then
    exit
  fi
fi
...
if ! CONVERTTO=$(${ZENITYWIN} -cancel-label "Abbrechen" -ok-label "Ok" --list --text="Zeile mit der Maus markieren ..." --title="Bitte Zielformat wählen" --column=ansi \
  asciidoc \
  asciidoc_legacy \
  asciidoctor \
  beamer \
...
if test -f "$SOURCEFILE.$CONVERTTO"; then
  if ! OVERWRITE=$(${ZENITYWIN} -cancel-label "Nein" -ok-label "Ja" --question --title="Information" --text="Die Datei: \"$SOURCEFILE.$CONVERTTO\" existiert schon, überschreiben ?"); then
    exit
  fi
fi
...
(
  ...
  if test ${PANEXIT} == "false"; then
    ${ZENITYWIN} -cancel-label "Abbrechen" -ok-label "Ok" --error --title="Fehler" --text="Fehler aufgetreten: ${PANOUTPUT}";
    # Exit from sub-shell, when error was raised.
    exit 1
  else
    ${ZENITYWIN} --info --title="Information" --text="Die Datei: \"$SOURCENAME\" wurde in das Format: \"$CONVERTTO\" konvertiert und gespeichert unter: \"$TARGETNAME\".";
    # exit
  fi
# SUBSHELL ---  END  ---
    ...
    --auto-close --title="Konvertierung läuft !" \
    --text="Bitte warten ..."
